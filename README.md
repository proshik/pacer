...
Сборочная стадия работает на платформе хоста и кросс-компилирует под целевую, поэтому образ под
`amd64` и `arm64` собирается без эмуляции: `docker buildx build --platform linux/amd64,linux/arm64 .`.
Публикация по тегу `v*` в CI собирает обе платформы.
