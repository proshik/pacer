...
На странице раздел «Сохранённые расчёты» и кнопка «Сохранить» появляются, только если запрос
`GET /api/v1/runs` с заголовком `tma` прошёл успешно. Вне Telegram и при 503 (нет `DB_PATH`)
страница выглядит как в обычном браузере.
