...
## Summary
- Состояние на 2026-09-16: выпущен v1.0.0 (тег и GitHub Release); этапы 1–3 закрыты, кроме
  окончательной проверки Mini App; к этапу 4 не приступали.
- Ждёт владельца проекта: повторный запуск Mini App, чтобы разобрать язык (сервер пишет
  `language_code` при входе); ручная проверка интерфейса; публикация образа v1.0.0 в Docker Hub —
  CI по тегу не запустится, пока Actions недоступны.
- Интерфейс: Vanilla JS и CSS без фреймворков; вне Telegram страница не делает внешних запросов.
- Развёртывание: образ собирает только серверный бинарь и встраивает закоммиченные
  wasm-артефакты; история расчётов в томе `/data`.
- Ворота качества: `go test -race`, `go vet`, golangci-lint v2.13, `./build.sh` на TinyGo,
  `node scripts/checks/all.mjs` (проверки страницы в headless Chrome) и `docker build .` —
...
- [x] Проверить живой запуск Mini App (через `setChatMenuButton`, без BotFather); вход с полем `signature` прошёл — оно участвует в проверке `hash`.
- [x] Таблица сплитов.
- [x] Предсказание времени по формулам Ригеля и Кэмерона.
- [x] VDOT и тренировочные зоны по Дэниелсу.
- [x] Карточка результата картинкой в чат: `pkg/card` (шрифты Go, кириллица без своего TTF), `GET /api/v1/card.png` и команда бота `/card`.
...
- [x] Сохранённые расчёты в обычном браузере: `localStorage`, не больше 50, как на сервере; в Mini App без серверной истории — тоже в браузере.
- [x] Меню команд бота в Telegram на двух языках.
- [x] Бот как вход в приложение: планы текстом без команды (`pkg/plan`), ответы с кнопками «Раскладка», «Прогноз», «Карточка», «Открыть в Pacer», кнопка «Открыть Pacer» в `/start`, ссылки `startapp` на странице.
- [ ] Инлайн-режим (`@PacerGoBot марафон 3:30` в любом чате) и `savePreparedInlineMessage` для кнопки «Поделиться» в Mini App.
...
- [x] Нелинейный слайдер дистанции: две трети дорожки — до 50 км с шагом 100 м, дальше до 500 км с шагом 1 км, риска на изломе.
...
- 2026-09-12: Сохранённые расчёты в обычном браузере. Два хранилища с общим интерфейсом: сервер — для вошедшего через Mini App, если сервер ведёт историю; иначе `localStorage` (`pacer.runs`, до 50), в том числе в Mini App при 503. Без доступного хранилища раздела нет. Сохраняется показанный план, а не сырые поля: при минутах «75» раньше ушло бы молча пересчитанное 2:15:39. Проверено в headless Chrome (19 проверок): сохранение, порядок, перезагрузка, открытие, удаление, предел 50, запрет хранилища, серверная история без записи в браузер, откат на браузер при 503; проверки языка, офлайна и ошибок проходят (39, 11, 17).
- 2026-09-12: Docker больше не пересобирает wasm: в образ уходят закоммиченные `assets/json.wasm`, `assets/wasm_exec.js` и `pkg/wasmcalc/calc.wasm` — проверено, оба файла байт в байт лежат в бинаре собранного образа. Стадия TinyGo из Dockerfile убрана, в CI после `./build.sh` добавлен `git diff --exit-code` по этим файлам: забытая пересборка теперь валит сборку, а не уезжает в прод.
- 2026-09-16: Релиз v1.0.0: `result-card` слита в `master`, тег и GitHub Release. Сборочная стадия Dockerfile кросс-компилирует (`--platform=$BUILDPLATFORM`), публикация по тегу собирает amd64 и arm64; локально оба образа собраны (22 и 9 с) и стартуют. Перед релизом: `go test -race`, vet, golangci-lint, `go build`, пять браузерных проверок (43, 11, 17, 19, 22), `docker build`; wasm-исходники с прошлой пересборки не менялись. Образ в Docker Hub не опубликован: Actions недоступны из-за биллинга.
- 2026-09-16: Живой запуск Mini App: туннель cloudflared и кнопка меню через `setChatMenuButton` (без BotFather). Вход прошёл, тема Telegram применилась. Страница открылась на английском — причина ждёт повторного входа: сервер теперь пишет в лог `language_code` и наличие поля `signature`.
- 2026-09-16: Слайдер дистанции стал нелинейным: первые две трети дорожки — 0–50 км с шагом 100 м (на телефоне ~210 м на пиксель вместо ~560), остальное — до 500 км с шагом 1 км; левый край — 100 м вместо недопустимого нуля; стрелки шагают на шаг своего участка; диктор слышит километры (`aria-valuetext`); излом отмечен риской «50 км». Своя шкала поверх нативного `range`, без библиотек. Новая проверка `scripts/checks/slider.mjs` — 22 проверки; остальные проходят (43, 11, 17, 19).
- 2026-09-16: Переключатель языка — пара `RU | EN` с `aria-pressed` вместо кнопки с названием другого языка. Заодно найдено, почему в Mini App он стоял ниже слова Pacer: резерв двух строк (`min-height: 2lh`) из исправления «разъезжающейся» страницы цеплял и абзац с логотипом; резерв оставлен подзаголовку и сноскам. Проверка выравнивания теперь меряет сам текст, а не блок — прежняя по блокам ошибку пропустила. Проверки: 43, 11, 17, 19.
- 2026-09-12: Браузерные проверки переехали из временного каталога сессии в репозиторий: `scripts/checks/` — `language.mjs`, `offline.mjs`, `validation.mjs`, `saved-runs.mjs` и общий `all.mjs`. Пути отвязаны от машины (`ASSETS`, `CHROME`, `CHECK_OUT`), скриншоты по умолчанию уходят во временный каталог. Прогон из репозитория даёт тот же счёт: 39, 11, 17 и 19.
- 2026-09-12: Карточка плана картинкой: `pkg/card` рисует PNG 1200×630 — слева дистанция, время и темп, справа раскладка по отсечкам с финишем. Шрифты Go (`golang.org/x/image/font/gofont`) дают кириллицу без своего TTF в репозитории; шаг отсечек подбирается под высоту колонки, отрисовка детерминирована. Два пути наружу: `GET /api/v1/card.png?d=&t=&l=` (те же параметры, что у превью, кэш на сутки) и команда бота `/card`, ради которой отправитель перестал быть привязан к тексту (`outgoing`: `textReply` и `photoReply`); подпись к фото — та же фраза словами. Команда добавлена в приветствие, в список доступных и в меню Telegram. Тесты: пакет (размер, палитра, наличие отсечек, детерминированность, отказ от нерисуемого плана), эндпоинт (PNG, язык, 400 на неверные параметры), бот (фото с подписью через поддельный сервер Telegram). Новые зависимости — `golang.org/x/image` и `golang.org/x/text`.
- 2026-09-12: Бот: английские ответы переписаны (были «empty arguments», «should be 2 arguments: (pace, dist) separated by a space») — теперь называют аргументы словами; тест на число аргументов проверяет смысл, а не строку. Добавлено меню команд `setMyCommands` на двух языках: без языка английское, с `ru` — русское; ошибка публикации только логируется.
- 2026-09-16: Бот стал входом в приложение. `pkg/plan` (свой разбор, выбран вместо библиотеки) читает план в свободной форме — «марафон 3:30», `10k 4:50`, `21,1 км 1:38:48`, мили, метры, старый синтаксис Go; часы или минуты, финиш или темп решает правдоподобие темпа. Сообщение без команды — тоже план. Ответ — HTML с кнопками: «Раскладка» и «Прогноз» переписывают сообщение таблицей (`editMessageText`), «Карточка» присылает фото, «Открыть в Pacer» открывает страницу на плане; `web_app` только в личке, в группах — обычная ссылка. `/start` — примеры и кнопка «Открыть Pacer». Ошибки называют, чего не хватает, и цитируют непонятые слова. Страница читает `tgWebAppStartParam` (`d42195_t12600`), новая проверка `startapp.mjs` — 18. Бот больше не держит `calculator.Engine`: он считает через `pkg/plan` и `pkg/card`, как и раньше карточка. Проверки: `go test -race`, vet, golangci-lint (0), `go build`, `docker build`, браузер 43, 11, 17, 19, 22, 18.
- 2026-09-16: Причина английского языка в живом Mini App найдена: Telegram прислал `language_code=en` — это язык приложения Telegram у пользователя, страница выбрала верно. Вход с `signature=true` прошёл, так что поле действительно участвует в проверке `hash`.
- 2026-09-16: Логотип в шапке: иконка приложения (32 px) и слово Pacer в 24 px вместо 16 px — прежнее терялось рядом с планом. Проверки страницы: 43, 11, 17, 19, 22, 18.
- 2026-09-16: Заголовок плана легче и мельче (600, 20–28 px вместо 700, 26–40 px): рядом с ним терялся логотип.
- 2026-09-22: Репозиторий пересоздан с переписанной историей (все коммиты — noreply-адрес, рабочих адресов нет) и открыт. Ручной релиз от `master`: workflow `release` — выбор patch/minor/major, проверки `ci.yml`, образ под amd64 и arm64, тег и черновик релиза.
- 2026-09-22: Первый запуск CI после открытия репозитория упал на проверке свежести wasm: артефакты собраны 2026-09-10 на Go 1.27.1, а CI брал Go 1.26.0 из `go.mod`; после смены `go 1.26` на `go 1.26.0` и обновления `x/sys` байты разошлись бы и на 1.27.1. В `go.mod` закреплён `toolchain go1.26.4`, TinyGo 0.42 поставлен локально, оба артефакта пересобраны (`json.wasm` 928 → 382 КБ, сборка воспроизводима). Проверки: `go test -race`, vet, сборка, браузер 43, 11, 17, 19, 22, 18.
- 2026-09-22: CI упал ещё раз уже на `json.wasm`: TinyGo 0.42 из Homebrew на macOS и TinyGo 0.42 под Linux дают разные байты при одном Go 1.26.4, а Linux-сборки на arm64 и amd64 совпадают с CI. `build.sh` на macOS теперь собирает бандл в контейнере `golang:<toolchain>` с TinyGo из deb-пакета; закоммичен бандл, побайтно равный CI (384 358 байт).
- 2026-09-22: Образ публикуется в GitHub Container Registry (`ghcr.io/proshik/pacer`) вместо Docker Hub — через `GITHUB_TOKEN`, без секретов; и ручной релиз, и тег, запушенный руками.
- 2026-09-22: Релиз v1.1.1 выпущен workflow `release`: образ `ghcr.io/proshik/pacer:v1.1.1` и `latest` (amd64, arm64), пакет публичный и привязан к репозиторию. Инструкции для агентов собраны в один `AGENTS.md` (русский, бывший `CLAUDE.md` плюс структура, стиль, тесты, коммиты и безопасность из прежнего английского `AGENTS.md`); `CLAUDE.md` только подключает его строкой `@AGENTS.md`. README: образ из ghcr, уточнено, где при сборке нужна эмуляция.
