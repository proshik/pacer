...
## Summary
- Состояние на 2026-09-12: исходный план (апрель 2026, разделы 1–11) выполнен; этапы 1–3 закрыты,
  кроме живого запуска Mini App. Ветка `stage-1-hygiene` слита в `master`, карточка результата
  лежит в ветке `result-card`; к этапу 4 не приступали.
- Ждёт владельца проекта: настройка Mini App в BotFather и живой запуск — он же подтвердит, что
  поле `signature` участвует в проверке `hash`; ручная проверка интерфейса; пуш `master` и
  ветки `result-card`.
